...
async function buildCheckoutFromItems({ userId, items, addressId, shippingAddress, couponCode, conn, lockProducts = false }) {
  ...
  // Enforce server-side minimum order value
...
async function creditReferralIfFirstOrder(userId, orderId) {
  try {
    const referral = await getReferralByReferee(userId);
    if (!referral || referral.credited) return;
    const REFERRAL_CREDIT = 50;
    await creditWallet(referral.referrerId, REFERRAL_CREDIT);
    await creditWallet(userId, REFERRAL_CREDIT);
...
async function notifyOrderConfirmation(userId, order) {
  ...
  // SMS — fire-and-forget
  const phone = user?.mobile || user?.phone || order?.shippingAddress?.phone;
  ...
  // Mark any abandoned cart as recovered — fire-and-forget
...
async function updateOrderStatus(id, status, trackingNumber = null, courierName = null) {
  ...
  await updateOrderStatusRepo(id, status, trackingNumber, courierName);
  const updatedOrder = await findOrderById(id);

  // Fire notifications — fire-and-forget
  findUserById(existing.userId).then((user) => {
    sendOrderStatusEmail(updatedOrder, user, status).catch(() => null);
    const phone = user?.mobile || user?.phone || updatedOrder?.shippingAddress?.phone;
    if (status === "shipped") {
      sendOrderShippedSms(phone, id, trackingNumber, courierName).catch(() => null);
      sendOrderShippedWhatsApp(phone, user?.name, id, trackingNumber, courierName).catch(() => null);
      sendPushToUser(existing.userId, { title: "Order Shipped 📦", body: `Order #${id} is on its way!`, url: "/my-orders" }).catch(() => null);
