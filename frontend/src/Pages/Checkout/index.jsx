...
const Checkout = () => {
  ...
  const handlePay = async () => {
    ...
    let orderData;
    try {
      orderData = await postData('/api/payment/create-order', { amount: total });
    } catch {
  ...
  const handlePaymentSuccess = async (razorpayResponse, rzpOrderId) => {
    ...
    try {
      ...
      // 2. Build order items from cartItems
      ...
      const orderRes = await postData('/api/order', {
        ...
        shippingAddress: selectedAddrObj
          ? {
              ...
              address: selectedAddrObj.flatHouse + (selectedAddrObj.areaStreet ? ', ' + selectedAddrObj.areaStreet : ''),
              city: selectedAddrObj.townCity,
              state: selectedAddrObj.state,
              postalCode: selectedAddrObj.pincode,
              country: selectedAddrObj.country || 'India',
            }
          : {},
        ...
        freeShippingMilestoneApplied: milestoneShippingFree,
      });
